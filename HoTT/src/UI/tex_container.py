###############################################
#                                             #
#      Container for a single latex file      #
#                                             #
###############################################
...
log_prefix: str = "[UI][Tex container]"


class TexContainer:
    """Basically a decorator for the latex parser"""
    ...
    def transform_ast(self, tree: Tree):
        ...
        if True:
            for rule in rules:
                ...
                elif rule_name == "construction":
                    self.add_construction(rule)
    ...
    def add_construction(self, construction: Tree):
        prefix = f"{log_prefix}[add_construction]({self.name})"
 
        # check integrity
        if True:
            # check if the construction contains a name, an id and an actual construction
            children_names = [c.data for c in construction.children]
            if len(construction.children) != 3 \
                    or "construction_id" not in children_names \
                    or "construction_name" not in children_names \
                    or "actual_construction" not in children_names:
                err_msg = f"Cannot retrieve construction from tree: a construction should have " + \
                    f"a construction_id, a construction_name and an actual_construction. Got : {children_names}"
                Logger.error(err_msg, prefix)
                return                

        # preprocess
        if True:
            construction_id = [c.children[0] for c in construction.children if c.data == "construction_id"][0]
            construction_name = [c.children[0] for c in construction.children if c.data == "construction_name"][0]
            actual_construction = [c.children for c in construction.children if c.data == "actual_construction"][0]

            # 'unwrap'
            construction_id = construction_id.children[0].value
            construction_name = construction_name.children[0].value
   
        # process
        if True:
            construction = Construction(construction_name, construction_id, actual_construction)
            self.contexts[f"{construction_id} ::: {construction_name}"] = construction
    
         # postprocess
        if True:
            Logger.info(f"'{construction_name}' ({construction_id}) added", prefix, 10)
